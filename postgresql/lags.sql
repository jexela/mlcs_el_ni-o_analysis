...
WITH window_correlations as (
						SELECT l.start_year,l.end_year,l.longditude,l.laditude,corr(l.oni,l.lagged_anomoly) as correlation, atan(corr(l.oni,l.lagged_anomoly)) as fisher_transformation,l.lag
	 ...
	 max_lags 			as(
						SELECT m.start_year,m.end_year,m.longditude,
							   ...
							   m.fisher_transformation_avg-m.fisher_transformation_stderror*1.65 as z_score_low,
							   m.fisher_transformation_avg+m.fisher_transformation_stderror*1.65 as z_score_high,
...
--Probability that the maximum is outside the unsafe area ,1-P(-Z=-1.24<x<Z=1.24) = 100%-78.502%=21.498%, whereby there are no places over 1979-1988,1980-1989,1990-1999,...,2012-2021 that do consist exclusively of NULL values.

CREATE TABLE plot_lags as
SELECT l.longditude,
	   l.laditude, 
	   CASE WHEN count(l.lag) >= 8 THEN percentile_disc(0.5) within group (order by l.lag) ELSE NULL END as "median max lag",
...
--	   percentile_disc(0.75) within group (order by l.lag) as "75th percentile",
	   ...
	   CASE WHEN count(l.lag)  >= 8 THEN count(l.lag) ELSE NULL END as "amount of lags"
FROM (SELECT lag_max_correlations.start_year,
			 ...
			 CASE WHEN (lag_max_correlations.max_correlation NOT BETWEEN tan(lag_max_correlations.z_score_low) AND tan(lag_max_correlations.z_score_high))
			 AND abs(lag_max_correlations.max_correlation)>=0.179 THEN lag_max_correlations.lag ELSE NULL END as lag,
...
--	   percentile_disc(0.5) within group (order by l.lag) "median max lag",
	   CASE WHEN count(l.lag) >= 8 THEN percentile_disc(0.75) within group (order by l.lag) - percentile_disc(0.25) within group (order by l.lag) ELSE NULL END as "interquartile range",
--	   percentile_disc(0.25) within group (order by l.lag) as "25th percentile",
--	   percentile_disc(0.75) within group (order by l.lag) as "75th percentile",
	   array_agg(l.lag) as "maximum lags over windows 1979-1988,1980-1989,...,2012-2021",
	   CASE WHEN count(l.lag)  >= 8 THEN count(l.lag) ELSE NULL END  as "amount of lags"
FROM (SELECT lag_max_correlations.start_year,
			 ...
			 lag_max_correlations.longditude, 
			 lag_max_correlations.laditude,
			 CASE WHEN (lag_max_correlations.max_correlation NOT BETWEEN tan(lag_max_correlations.z_score_low) AND tan(lag_max_correlations.z_score_high)) AND abs(lag_max_correlations.max_correlation)>=0.179 THEN lag_max_correlations.lag ELSE NULL END as lag,
			 lag_max_correlations.z_score,
			 lag_max_correlations.z_score_low,
			 lag_max_correlations.z_score_high,
			 lag_max_correlations.stderror FROM lag_max_correlations as lag_max_correlations) as l 
