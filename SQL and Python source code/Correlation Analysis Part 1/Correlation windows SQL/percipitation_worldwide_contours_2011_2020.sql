...
CREATE TABLE period_2011_2020_countours (longditude,laditude,correlation) as
...
FROM (SELECT t.longditude,t.laditude, t.correlation,t_value,z_one,p_value
	   FROM (SELECT o.longditude, o.laditude, corr(o.percipitation_anomaly,o.oni) as "correlation"
			 FROM (SELECT * FROM oni WHERE oni.year BETWEEN 2011 AND 2020) as o
...
--DROP TABLE IF EXISTS period_2011_2020_countours_adj;

CREATE TABLE period_2011_2020_countours_adj as 
WITH RECURSIVE start as (
		SELECT p.*, p.p_value as p_value_adj
		FROM period_2011_2020_countours as p
		WHERE p.p_value = (SELECT max(p.p_value) as p_value_adj FROM period_2011_2020_countours as p)
	...
	UNION
		...
		FROM period_2011_2020_countours as p, start as s
...
FROM (SELECT CASE WHEN temp.p_value_adj < 0.05 THEN temp.correlation ELSE 0 END as correlation FROM period_2011_2020_countours_adj as temp) as p
WHERE p.correlation <> 0;

-- Cutoff line =   0.274574866120598767, which corresponds to the smallest absolute correlation with p_value_adj < 0.05
--TABLE period_2011_2020_countours OFFSET 10000;

\copy period_2011_2020_countours_adj TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Benjamini_Hochberg/csv_contours/period_2011_2020_countours_adj.csv' DELIMITER ';' CSV HEADER;
...
DROP TABLE IF EXISTS blues_2011_2020;

CREATE TABLE blues_2011_2020 (longditude,laditude,correlation) as 
WITH RECURSIVE shapes (longditude,laditude,correlation) as (
		SELECT p.longditude,p.laditude,p.correlation 
		FROM period_2011_2020_countours_adj as p 
		WHERE p.laditude = 1.25 AND p.longditude = 126.25
	UNION 
		SELECT p.longditude,p.laditude,p.correlation
		FROM (SELECT * FROM period_2011_2020_countours_adj as temp WHERE temp.correlation <= -  0.274574) as p, shapes as s 
		WHERE (s.laditude+2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation <= -  0.274574) OR 
			  (s.laditude+2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation <= -  0.274574) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation <= -  0.274574) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation <= -  0.274574) OR
			  (s.laditude+2.5=p.laditude AND s.longditude=p.longditude     AND p.correlation <= -  0.274574) OR
			  (s.laditude-2.5=p.laditude AND s.longditude=p.longditude     AND p.correlation <= -  0.274574) OR
		      (s.longditude+2.5=p.longditude AND s.laditude=p.laditude     AND p.correlation <= -  0.274574) OR
		      (s.longditude-2.5=p.longditude AND s.laditude=p.laditude	   AND p.correlation <= -  0.274574)
)
...
SELECT * 
FROM(
SELECT p.longditude,p.laditude, CASE WHEN abs(p.correlation) <   0.274574 THEN p.correlation ELSE 0 END
FROM period_2011_2020_countours_adj as p
...
WITH RECURSIVE shapes2 (longditude,laditude,correlation) as (
	...
	UNION 
		SELECT p.longditude,p.laditude,p.correlation
		FROM (SELECT * FROM period_2011_2020_countours_adj as temp WHERE temp.correlation >=   0.274574) as p, shapes2 as s 
		WHERE (s.laditude+2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation >=   0.274574) OR 
			  (s.laditude+2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation >=   0.274574) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation >=   0.274574) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation >=   0.274574) OR
...
\copy reds_2011_2020 TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Benjamini_Hochberg/reds_2011_2020.csv' DELIMITER ';' CSV HEADER;
\copy reds_2011_2020 TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Shapes0.05/csv/reds_2011_2020.csv' DELIMITER ';' CSV HEADER;
...
FROM reds_2011_2020 as r,
	 ...
	 LATERAL (SELECT horizontal_distance*vertical_distance as square_miles) square_miles
WHERE r.correlation >=   0.274574) as s;
