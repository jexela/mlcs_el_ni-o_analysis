...
CREATE TABLE period_2012_2021_countours (longditude,laditude,correlation) as
...
--Calculate correlation coefficient countours between oni values and global precipitation for every location for periods of 10 years (111 months, 109 degrees of freedom), in a 1 year cycle. 1979-1988,1980-1989,1981-1990,...
...
FROM (SELECT t.longditude,t.laditude, t.correlation,t_value,z_one,p_value
	   FROM (SELECT o.longditude, o.laditude, corr(o.percipitation_anomaly,o.oni) as "correlation"
			 FROM (SELECT * FROM oni WHERE oni.year BETWEEN 2012 AND 2021) as o
			 GROUP BY (o.longditude,o.laditude)
			 ORDER BY o.laditude ASC,o.longditude ASC) as t,
			 LATERAL (SELECT sqrt(111-2)*t.correlation/(sqrt(1-power(t.correlation,2))) as t_value) t_value,
			 LATERAL (SELECT abs(t_value)*(1-1/(4*109))*power((1+(power(t_value,2)/(2*109))),-0.5) as z_one) z_one,
...
--DROP TABLE IF EXISTS period_2012_2021_countours_adj;

CREATE TABLE period_2012_2021_countours_adj as 
WITH RECURSIVE start as (
		SELECT p.*, p.p_value as p_value_adj
		FROM period_2012_2021_countours as p
		WHERE p.p_value = (SELECT max(p.p_value) as p_value_adj FROM period_2012_2021_countours as p)
	...
	UNION
		...
		FROM period_2012_2021_countours as p, start as s
...
FROM (SELECT CASE WHEN temp.p_value_adj < 0.05 THEN temp.correlation ELSE 0 END as correlation FROM period_2012_2021_countours_adj as temp) as p
WHERE p.correlation <> 0;

-- Cutoff line =   0.28836866111598767, which corresponds to the smallest absolute correlation with p_value_adj < 0.05
--TABLE period_2012_2021_countours OFFSET 10000;

\copy period_2012_2021_countours_adj TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Benjamini_Hochberg/csv_contours/period_2012_2021_countours_adj.csv' DELIMITER ';' CSV HEADER;

/* Test approximations to the t distribution https://www.researchgate.net/publication/262689541_Approximations_to_the_t-distribution with r=829*sqrt(30187241)/(30187241) corresponding to a two tail area of 0.05 with 111 observations (109 degrees of freedom)
...
FROM (SELECT 99/sqrt(304801) as correlation) as t,
	  LATERAL (SELECT sqrt(111-2)*t.correlation/(sqrt(1-power(t.correlation,2))) as t_value) t_value,
	  LATERAL (SELECT abs(t_value)*(
1-1/(4*109))*power((1+(power(t_value,2)/(2*109))),-0.5) as z_one) z_one,
...
DROP TABLE IF EXISTS blues_2012_2021;

CREATE TABLE blues_2012_2021 (longditude,laditude,correlation) as 
WITH RECURSIVE shapes (longditude,laditude,correlation) as (
		SELECT p.longditude,p.laditude,p.correlation 
		FROM period_2012_2021_countours_adj as p 
		WHERE p.laditude = 1.25 AND p.longditude = 126.25
	UNION 
		SELECT p.longditude,p.laditude,p.correlation
		FROM (SELECT * FROM period_2012_2021_countours_adj as temp WHERE temp.correlation <= -  0.28836) as p, shapes as s 
		WHERE (s.laditude+2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation <= -  0.28836) OR 
			  (s.laditude+2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation <= -  0.28836) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation <= -  0.28836) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation <= -  0.28836) OR
			  (s.laditude+2.5=p.laditude AND s.longditude=p.longditude     AND p.correlation <= -  0.28836) OR
			  (s.laditude-2.5=p.laditude AND s.longditude=p.longditude     AND p.correlation <= -  0.28836) OR
		      (s.longditude+2.5=p.longditude AND s.laditude=p.laditude     AND p.correlation <= -  0.28836) OR
		      (s.longditude-2.5=p.longditude AND s.laditude=p.laditude	   AND p.correlation <= -  0.28836)
)
...
SELECT * 
FROM(
SELECT p.longditude,p.laditude, CASE WHEN abs(p.correlation) <   0.28836 THEN p.correlation ELSE 0 END
FROM period_2012_2021_countours_adj as p
...
WITH RECURSIVE shapes2 (longditude,laditude,correlation) as (
	...
	UNION 
		SELECT p.longditude,p.laditude,p.correlation
		FROM (SELECT * FROM period_2012_2021_countours_adj as temp WHERE temp.correlation >=   0.28836) as p, shapes2 as s 
		WHERE (s.laditude+2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation >=   0.28836) OR 
			  (s.laditude+2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation >=   0.28836) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation >=   0.28836) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation >=   0.28836) OR
...
\copy reds_2012_2021 TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Benjamini_Hochberg/reds_2012_2021.csv' DELIMITER ';' CSV HEADER;
\copy reds_2012_2021 TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Shapes0.05/csv/reds_2012_2021.csv' DELIMITER ';' CSV HEADER;
...
FROM reds_2012_2021 as r,
	 ...
	 LATERAL (SELECT horizontal_distance*vertical_distance as square_miles) square_miles
WHERE r.correlation >=   0.28836) as s;
