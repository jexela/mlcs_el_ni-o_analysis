...
CREATE TABLE period_1989_1998_countours (longditude,laditude,correlation) as
...
--DROP TABLE IF EXISTS period_1989_1998_countours_adj;

CREATE TABLE period_1989_1998_countours_adj as 
WITH RECURSIVE start as (
		SELECT p.*, p.p_value as p_value_adj
		FROM period_1989_1998_countours as p
		WHERE p.p_value = (SELECT max(p.p_value) as p_value_adj FROM period_1989_1998_countours as p)
	...
	UNION
		...
		FROM period_1989_1998_countours as p, start as s
...
FROM (SELECT CASE WHEN temp.p_value_adj < 0.05 THEN temp.correlation ELSE 0 END as correlation FROM period_1989_1998_countours_adj as temp) as p
WHERE p.correlation <> 0;

-- Cutoff line =   0.2517877866120598767, which corresponds to the smallest absolute correlation with p_value_adj < 0.05
--TABLE period_1989_1998_countours OFFSET 10000;

\copy period_1989_1998_countours_adj TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Benjamini_Hochberg/csv_contours/period_1989_1998_countours_adj.csv' DELIMITER ';' CSV HEADER;
...
DROP TABLE IF EXISTS blues_1989_1998;

CREATE TABLE blues_1989_1998 (longditude,laditude,correlation) as 
WITH RECURSIVE shapes (longditude,laditude,correlation) as (
		SELECT p.longditude,p.laditude,p.correlation 
		FROM period_1989_1998_countours_adj as p 
		WHERE p.laditude = 1.25 AND p.longditude = 126.25
	UNION 
		SELECT p.longditude,p.laditude,p.correlation
		FROM (SELECT * FROM period_1989_1998_countours_adj as temp WHERE temp.correlation <= -  0.2517877) as p, shapes as s 
...
DROP TABLE IF EXISTS reds_1989_1998;

CREATE TABLE reds_1989_1998 (longditude,laditude,correlation) as 
WITH RECURSIVE shapes2 (longditude,laditude,correlation) as (
	...
	UNION 
		SELECT p.longditude,p.laditude,p.correlation
		FROM (SELECT * FROM period_1989_1998_countours_adj as temp WHERE temp.correlation >=   0.2517877) as p, shapes2 as s 
		WHERE (s.laditude+2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation >=   0.2517877) OR 
			  (s.laditude+2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation >=   0.2517877) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation >=   0.2517877) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation >=   0.2517877) OR
...
\copy reds_1989_1998 TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Benjamini_Hochberg/reds_1989_1998.csv' DELIMITER ';' CSV HEADER;
\copy reds_1989_1998 TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Shapes0.05/csv/reds_1989_1998.csv' DELIMITER ';' CSV HEADER;
