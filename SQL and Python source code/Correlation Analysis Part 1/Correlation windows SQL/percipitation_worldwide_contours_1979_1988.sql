...
--Calculate precipitation anomolies for each location by subtracting long term month precipitation averages (1979-2021) from measured averaged precipitation 
	anomalies as (
		...
		ORDER BY t.id),
--Join precipitation anamolies table with oci index table to match every location with the corresponding oni values
	oni as(
		SELECT a.*,e.oni
...
CREATE TABLE detrend as (
SELECT d.longditude,d.laditude,a.year,a.month,a.percipitation_anomaly, a.percipitation_anomaly-prediction as difference,prediction,d.trend,
	   CASE WHEN d.trend = 'trend exist' THEN a.percipitation_anomaly-prediction
			ELSE a.percipitation_anomaly END AS precip_anomaly,
			RANK () OVER (PARTITION BY a.longditude, a.laditude ORDER BY a.year,a.month)
FROM(
SELECT c.longditude,c.laditude,c.intercept, c.slope,
	   c.slope/(sqrt(1::numeric/503)*(c.y_stddev/c.x_stddev)::numeric*sqrt(1-c.correlation*c.correlation)::numeric) as test_statistic,
	   CASE WHEN abs(c.slope/(sqrt(1::numeric/503)*(c.y_stddev/c.x_stddev)::numeric*sqrt(1-c.correlation*c.correlation)::numeric)) > 1.975 THEN 'trend exists'
	   ELSE 'trend does not exist' END as trend
FROM (
		SELECT b.longditude,
			   b.laditude,
			   regr_intercept(b.percipitation_anomaly,b.rank) as intercept,
			   regr_slope(b.percipitation_anomaly,b.rank) as slope, 
			   corr(b.rank,b.percipitation_anomaly) as correlation,
			   stddev(b.rank) as x_stddev ,
			   stddev(b.percipitation_anomaly) as y_stddev
		FROM(SELECT a.longditude, a.laditude, RANK () OVER (PARTITION BY a.longditude, a.laditude ORDER BY a.year,a.month) as rank, a.percipitation_anomaly,a.year,a.month
			 FROM anomalies as a ORDER BY a.longditude,a.laditude,RANK () OVER (PARTITION BY a.longditude, a.laditude ORDER BY a.year,a.month)) as b
GROUP BY b.longditude,b.laditude) as c) as d, anomalies as a,
LATERAL (SELECT (d.intercept+d.slope*(RANK () OVER (PARTITION BY a.longditude, a.laditude ORDER BY a.year,a.month))) as prediction) prediction
WHERE a.longditude = d.longditude AND a.laditude = d.laditude);

SELECT e.*
FROM (
SELECT d.*, d.precip_anomaly-d.prediction as detrend
FROM detrend as d 
WHERE d.trend = 'trend exists') as e
ORDER BY abs(difference) DESC LIMIT 10;
--WHERE c.trend <> 'steady';
*/
...
FROM (SELECT t.longditude,t.laditude, t.correlation,t_value,z_one,p_value
	   FROM (SELECT o.longditude, o.laditude, corr(o.percipitation_anomaly,o.oni) as "correlation"
			 FROM (SELECT * FROM oni WHERE oni.year BETWEEN 1979 AND 2021) as o
			 GROUP BY (o.longditude,o.laditude)
			 ORDER BY o.laditude ASC,o.longditude ASC) as t,
			 LATERAL (SELECT sqrt(512-2)*t.correlation/(sqrt(1-power(t.correlation,2))) as t_value) t_value,
			 LATERAL (SELECT abs(t_value)*(1-1/(4*510))*power((1+(power(t_value,2)/(2*510))),-0.5) as z_one) z_one,
			 LATERAL (SELECT 2*(1-power(1+exp(0.000345*power(z_one,5)-0.069547*power(z_one,3)-1.604326*z_one),-1)) as p_value) p_value) as p
WINDOW win as (ORDER BY abs(p.correlation) ROWS BETWEEN 1 PRECEDING AND CURRENT ROW)
;

--DROP TABLE IF EXISTS period_1979_1988_countours_adj;

CREATE TABLE period_1979_1988_countours_adj as 
WITH RECURSIVE start as (
		SELECT p.*, p.p_value as p_value_adj
		FROM period_1979_1988_countours as p
		WHERE p.p_value = (SELECT max(p.p_value) as p_value_adj FROM period_1979_1988_countours as p)
	...
	UNION
		...
		FROM period_1979_1988_countours as p, start as s
...
FROM (SELECT CASE WHEN temp.p_value_adj < 0.05 THEN temp.correlation ELSE 0 END as correlation FROM period_1979_1988_countours_adj as temp) as p
WHERE p.correlation <> 0;

-- Cutoff line =  0.25192413288650173, which corresponds to the smallest absolute correlation with p_value_adj < 0.05
--TABLE period_1979_1988_countours OFFSET 10000;

\copy period_1979_1988_countours_adj TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Benjamini_Hochberg/csv_contours/period_1979_1988_countours_adj.csv' DELIMITER ';' CSV HEADER;
...
DROP TABLE IF EXISTS blues_1979_1988;

CREATE TABLE blues_1979_1988 (longditude,laditude,correlation) as 
WITH RECURSIVE shapes (longditude,laditude,correlation) as (
		SELECT p.longditude,p.laditude,p.correlation 
		FROM period_1979_1988_countours_adj as p 
		WHERE p.laditude = 1.25 AND p.longditude = 126.25
	UNION 
		SELECT p.longditude,p.laditude,p.correlation
		FROM (SELECT * FROM period_1979_1988_countours_adj as temp WHERE temp.correlation <= -0.2519) as p, shapes as s 
		WHERE (s.laditude+2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation <= -0.2519) OR 
			  (s.laditude+2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation <= -0.2519) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation <= -0.2519) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation <= -0.2519) OR
			  (s.laditude+2.5=p.laditude AND s.longditude=p.longditude     AND p.correlation <= -0.2519) OR
			  (s.laditude-2.5=p.laditude AND s.longditude=p.longditude     AND p.correlation <= -0.2519) OR
		      (s.longditude+2.5=p.longditude AND s.laditude=p.laditude     AND p.correlation <= -0.2519) OR
		      (s.longditude-2.5=p.longditude AND s.laditude=p.laditude	   AND p.correlation <= -0.2519)
)
...
SELECT * 
FROM(
SELECT p.longditude,p.laditude, CASE WHEN abs(p.correlation) < 0.2519 THEN p.correlation ELSE 0 END
FROM period_1979_1988_countours_adj as p
...
WITH RECURSIVE shapes2 (longditude,laditude,correlation) as (
	...
	UNION 
		SELECT p.longditude,p.laditude,p.correlation
		FROM (SELECT * FROM period_1979_1988_countours_adj as temp WHERE temp.correlation >= 0.2519) as p, shapes2 as s 
		WHERE (s.laditude+2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation >= 0.2519) OR 
			  (s.laditude+2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation >= 0.2519) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude-2.5=p.longditude AND p.correlation >= 0.2519) OR 
			  (s.laditude-2.5=p.laditude AND s.longditude+2.5=p.longditude AND p.correlation >= 0.2519) OR
...
\copy reds_1979_1988 TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Benjamini_Hochberg/reds_1979_1988.csv' DELIMITER ';' CSV HEADER;
\copy reds_1979_1988 TO 'C:/Users/Alexej/Desktop/Bachelorarbeit/climatology 1981-2010/Shapes0.05/csv/reds_1979_1988.csv' DELIMITER ';' CSV HEADER;
...
FROM reds_1979_1988 as r,
	 ...
	 LATERAL (SELECT horizontal_distance*vertical_distance as square_miles) square_miles
WHERE r.correlation >= 0.2519) as s;